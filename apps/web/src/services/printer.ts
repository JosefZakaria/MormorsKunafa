...
/** Epson ePOS-Print: endast align (samma stil som fungerande testutskrift). */
function textLine(content: string, align: 'left' | 'center' | 'right' = 'left'): string {
  const body = escapeXml(content);
  return `<text align="${align}">${body}&#10;</text>`;
}
...
/**
 * Testar anslutningen genom en minimal utskrift (samma XML-stil som kvitto).
 */
