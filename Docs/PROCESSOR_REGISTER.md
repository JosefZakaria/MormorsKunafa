...
| Sinch | Phone number, bounded transactional SMS text | Processor | DPA, confirmed EU Conversation API project/region, log retention, subprocessors |
| Stripe | Customer email, server-created line items/amount, order ID metadata, payment/refund identifiers | Determine independent-controller/processor split | Current services agreement/DPA, role statement, transfer safeguards, retention, subprocessors |
| Swish/connected bank | Phone alias, amount, currency, order reference and payment/refund identifiers over mTLS | Determine independent-controller/recipient role | Merchant agreement, role statement, retention and recipient chain |
