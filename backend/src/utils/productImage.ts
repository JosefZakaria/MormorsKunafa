...
const WP_BASENAME_TO_LOCAL: Record<string, string> = {
...
function wpRelativePath(url: string): string | null {
...
export function resolveProductImage(
  ...
  if (slugKey) {
    for (const [key, path] of Object.entries(PRODUCT_IMAGE_BY_SLUG)) {
      if (slugKey.startsWith(key) || key.startsWith(slugKey)) return path;
    }
  }
